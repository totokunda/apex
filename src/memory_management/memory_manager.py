...
class MemoryManager:
    ...
    def _selective_offload(self):
        ...
        for module_id, module_ref in self._managed_modules.items():
            ...
            if (
                time_since_forward > self.config.time_since_forward_threshold
            ):  # 2 second threshold
    ...
    def wrap_module(
        self, module: nn.Module, module_id: Optional[str] = None
    ) -> OffloadableModule:
    ...
    def wrap_model(
        self, model: nn.Module, layer_types: Optional[List[type]] = None
    ) -> nn.Module:
        ...
        wrapped_model = wrap_model_layers(
            model, self.config, self.memory_monitor, layer_types
        )
    ...
    def _get_module_summary(self) -> Dict[str, Any]:
        ...
        summary["total_memory_mb"] = (
            summary["loaded_memory_mb"] + summary["offloaded_memory_mb"]
        )
    ...
    def cleanup(self):
        ...
        if (
            hasattr(self.offload_strategy, "disk_strategy")
            and self.offload_strategy.disk_strategy
        ):
