...
# Don't set device at import time - let it be determined at runtime
# set_torch_device()  # REMOVED - causes MPS issues in celery workers
...
class AnimeSegmentation:
    def __init__(self, ckpt_path):
        ...
        # Get device at runtime, not import time
    ...
    def get_mask(self, input_img, s=640):
        ...
        if s == 0:
            ...
            tmpImg = torch.from_numpy(img_input).float().to(self.device)
            with torch.no_grad():
