...
class LineartDetector(ToMixin, BasePreprocessor):
    ...
    def from_pretrained(cls, pretrained_model_or_path=HF_MODEL_NAME, filename="sk_model.pth", coarse_filename="sk_model2.pth"):
        ...
        coarse_model.load_state_dict(torch.load(coarse_model_path, map_location=torch.device('cpu'), weights_only=False))
        coarse_model.eval()
        coarse_model.to('cpu')

        return cls(model, coarse_model)
    
    def process(self, input_image: InputImage, coarse=False, detect_resolution=512, upscale_method="INTER_CUBIC", **kwargs) -> OutputImage:
