...
from src.auxillary.util import get_upscale_method, HWC3
...
class TileDetector(BasePreprocessor):
    ...
    def process(self, input_image: InputImage, pyrUp_iters=3, upscale_method="INTER_AREA", **kwargs) -> OutputImage:
...
class TTPlanet_Tile_Detector_GF(BasePreprocessor):
    ...
    def process(self, input_image: InputImage, scale_factor=2.0, blur_strength=2.0, radius=5, eps=0.2, **kwargs) -> OutputImage:
        input_image = self._load_image(input_image)
...
class TTPLanet_Tile_Detector_Simple(BasePreprocessor):
    ...
    def process(self, input_image: InputImage, scale_factor=2.0, blur_strength=2.0, **kwargs) -> OutputImage:
        input_image = self._load_image(input_image)
