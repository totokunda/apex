...
import random
from src.preprocess.diffusion_edge.taming.modules.losses.vqperceptual import *
...
def linear_beta_schedule(timesteps):
    ...
    beta_end = scale * 0.02
    return torch.linspace(beta_start, beta_end, timesteps, dtype = torch.float64 if torch.cuda.is_available() else torch.float32)
...
def cosine_beta_schedule(timesteps, s = 0.008):
    ...
    """
    steps = timesteps + 1
    x = torch.linspace(0, timesteps, steps, dtype = torch.float64 if torch.cuda.is_available() else torch.float32)
    ...
    def sample_fn(self, shape, up_scale=1, unnormalize=True, cond=None, denoise=False):
        ...
        for i, time_step in enumerate(time_steps):
            ...
            if i == time_steps.shape[0] - 1:
                s = cur_time
            if cond is not None:
                pred = self.model(img, cur_time, cond.to(device))
            else:
                pred = self.model(img, cur_time)
            # C, noise = pred.chunk(2, dim=1)
            C, noise = pred[:2]
