...
try:
    ...
    class Model:
        def __init__(self, *args, **kwargs):
            raise ImportError("GroundingDINO model could not be initialized because the package is not installed.")
        def predict_with_classes(self, *args, **kwargs):
            raise ImportError("GroundingDINO model could not be used because the package is not installed.")
...
class GDINOPreprocessor(BasePreprocessor):
    def __init__(self, config_path, model_path, save_path=DEFAULT_PREPROCESSOR_SAVE_PATH, device='cuda', box_threshold=0.25, text_threshold=0.2, iou_threshold=0.5, use_nms=True, **kwargs):
        super().__init__(model_path=model_path, save_path=save_path, preprocessor_type=PreprocessorType.IMAGE, **kwargs)
        ...
        self.model = Model(model_config_path=config_path,
    ...
    def __call__(self, image, classes=None, caption=None, **kwargs):
        ...
        if classes is not None:
            ...
            class_names = phrases.tolist() if isinstance(phrases, np.ndarray) else phrases
        else:
