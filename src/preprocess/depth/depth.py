...
class MidasDepthPreprocessor(BasePreprocessor):
    def __init__(self, model_path: str, save_path: str, model_type: str = 'dpt_hybrid', **kwargs):
        super().__init__(model_path, save_path, preprocessor_type=PreprocessorType.IMAGE, **kwargs)
        self.midas_inference = MiDaSInference(model_type=model_type, model_path=model_path)
        
    @torch.no_grad()
    @torch.inference_mode()
    def __call__(self, image: Union[Image.Image, List[Image.Image], List[str], str, np.ndarray, torch.Tensor]):
...
class DepthAnythingV2Preprocessor(BasePreprocessor):
    def __init__(self, model_path: str, save_path: str, device: str = 'cuda', encoder: str = 'vitl', features: int = 256, out_channels: List[int] = [256, 512, 1024, 1024], **kwargs):
        super().__init__(model_path, save_path, preprocessor_type=PreprocessorType.IMAGE, **kwargs)
        self.model = DepthAnythingV2(encoder=encoder, features=features, out_channels=out_channels).to(device)
        self.model.load_state_dict(torch.load(model_path, map_location=device, mmap=True))
        self.model.eval()
