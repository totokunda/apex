...
class FrameRefExtractPreprocessor(BasePreprocessor):
    def __init__(self, ref_cfg=None, ref_num=1, ref_color=127.5, return_dict=True, return_mask=True, **kwargs):
        super().__init__(**kwargs)
        # first / last / firstlast / random
        self.ref_cfg = ref_cfg if ref_cfg is not None else [
    ...
    def __call__(self, frames, ref_cfg=None, ref_num=None, return_mask=None, return_dict=None):
...
class FrameRefExpandPreprocessor(BasePreprocessor):
    def __init__(self, ref_color=127.5, return_mask=True, return_dict=True, mode="firstframe", **kwargs):
        super().__init__(**kwargs)
    ...
    def __call__(self, image=None, image_2=None, frames=None, frames_2=None, mode=None, expand_num=None, return_mask=None, return_dict=None):
        mode = mode if mode is not None else self.mode
        return_mask = return_mask if return_mask is not None else self.return_mask
        return_dict = return_dict if return_dict is not None else self.return_dict

        if 'frame' in mode:
            frames = [image] if image is not None and not isinstance(frames, list) else frames
            frames_2 = [image_2] if image_2 is not None and not isinstance(image_2, list) else frames_2
    ...
    def __str__(self):
        return f"FrameRefExpandPreprocessor(mode={self.mode}, ref_color={self.ref_color})"
