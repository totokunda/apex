...
class CompositionPreprocessor(BasePreprocessor):
    ...
    def __call__(
        ...
        masks_2: List[Union[Image.Image, np.ndarray]],
    ...
    def __call__(
        ...
        for image in images:
            ret_one_data = self.sbjref_ins(
                image=image, mode=mode, return_mask=return_mask, mask_cfg=mask_cfg
            )
    ...
    def __call__(
        ...
        if images is not None:
            ...
            ret_ref_data = self.ref_ins(
                images=images, mode=mode, return_mask=return_mask, mask_cfg=mask_cfg
            )
...
class ExpandAnythingPreprocessor(BasePreprocessor):
    def __init__(
        ...
        frameref_config: Dict,
        outpainting_config: Dict,
        **kwargs,
    ):
        super().__init__(preprocessor_type=PreprocessorType.IMAGE, **kwargs)
    ...
    def __call__(
        ...
        outpainting_data = self.outpainting_ins(
            expand_image, expand_ratio=expand_ratio, direction=direction
        )
        ...
        frameref_data = self.frameref_ins(
            outpainting_image, mode=mode_list[0], expand_num=expand_num
        )
...
class MoveAnythingPreprocessor(BasePreprocessor):
    ...
    def __call__(
        ...
        ret_layout_data = self.layout_bbox_ins(
            bbox, frame_size=frame_size, num_frames=expand_num, label=label
        )
        ...
        ret_data = {"frames": out_frames, "masks": out_mask}
        return ret_data
