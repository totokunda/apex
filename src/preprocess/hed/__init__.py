...
from src.preprocess.util import HWC3, nms, resize_image_with_pad, safe_step, custom_hf_download, HF_MODEL_NAME
...
class HEDdetector(ToMixin, BasePreprocessor):
    ...
    def from_pretrained(cls, pretrained_model_or_path=HF_MODEL_NAME, filename="ControlNetHED.pth"):
        ...
        netNetwork.load_state_dict(torch.load(model_path, map_location='cpu', weights_only=False))
        netNetwork.float().eval()
        netNetwork.to('cpu')  # Explicitly move to CPU

        return cls(netNetwork)
    
    def process(self, input_image: InputImage, detect_resolution=512, safe=False, scribble=False, upscale_method="INTER_CUBIC", **kwargs) -> OutputImage:
