...
class Pose2dDetector(BasePreprocessor):
    ...
    def from_pretrained(cls, 
        ...
        checkpoint = custom_hf_download(pretrained_model_or_path, filepath, subfolder="process_checkpoint/pose2d")
        det_checkpoint = custom_hf_download(det_pretrained_model_or_path, det_filepath, subfolder="process_checkpoint/det")
        return cls(checkpoint, det_checkpoint)
    
    def process(self, input_image: InputImage, upscale_method="INTER_CUBIC", mode="pose", target_size=(480, 832), **kwargs) -> OutputImage:
