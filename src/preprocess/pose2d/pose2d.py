...
from src.preprocess.pose2d.pose2d_utils import (
    read_img,
    box_convert_simple,
    bbox_from_detector,
    crop,
...
class SimpleOnnxInference(object):
    def __init__(self, checkpoint, device=get_torch_device(), reverse_input=False, **kwargs):
...
class Yolo(SimpleOnnxInference):
    def __init__(self, checkpoint, device=get_torch_device(), threshold_conf=0.05, threshold_multi_persons=0.1, input_resolution=(640, 640), threshold_iou=0.5, threshold_bbox_shape_ratio=0.4, cat_id=[1], select_type='max', strict=True, sorted_func=None, **kwargs):
...
class ViTPose(SimpleOnnxInference):
    def __init__(self, checkpoint, device=get_torch_device(), **kwargs):
...
class Pose2d:
    def __init__(self, checkpoint, detector_checkpoint=None, device=get_torch_device(), **kwargs):
    ...
    def __call__(
        ...
        **kwargs
        ...
        if self.detector is not None:
            ...
            bbox = self.detector(img[None], shape[None])[0][0]["bbox"]
        img, center, scale = self.model.preprocess(image, bbox)
        kp2ds = self.model(img[None], center[None], scale[None])
