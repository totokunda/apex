...
def custom_hf_download(pretrained_model_or_path, filename, ckpts_dir=annotator_ckpts_path, subfolder='', use_symlinks=USE_SYMLINKS, repo_type="model"):
...
class OpenposeDetector:
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_or_path=HF_MODEL_NAME, filename="body_pose_model.pth", hand_filename="hand_pose_model.pth", face_filename="facenet.pth", save_path=DEFAULT_PREPROCESSOR_SAVE_PATH):
        ...
        body_model_path = custom_hf_download(pretrained_model_or_path, filename, subfolder=subfolder, ckpts_dir=save_path)
        hand_model_path = custom_hf_download(pretrained_model_or_path, hand_filename, subfolder=subfolder, ckpts_dir=save_path)
        face_model_path = custom_hf_download(face_pretrained_model_or_path, face_filename, subfolder=subfolder, ckpts_dir=save_path)
    ...
    def __call__(self, input_image, detect_resolution=512, include_body=True, include_hand=True, include_face=True,  output_type="pil", image_and_json=False, upscale_method="INTER_CUBIC", xinsr_stick_scaling=False, **kwargs):
