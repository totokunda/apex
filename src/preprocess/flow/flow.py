...
class FlowPreprocessor(BasePreprocessor):
    def __init__(self, model_path: str=None, save_path: str=DEFAULT_PREPROCESSOR_SAVE_PATH, device: str = 'cuda', **kwargs):
        ...
        self.model.load_state_dict({k.replace('module.', ''): v for k, v in torch.load(self.model_path, map_location="cpu", weights_only=True).items()})
    ...
    def __call__(self, frames, *args, **kwargs):
        ...
        frames = [np.array(frame).astype(np.uint8).transpose(2, 0, 1).float()[None].to(self.device) for frame in frames]
