...
from src.preprocess.util import resize_image_with_pad, custom_hf_download, UNIMATCH_MODEL_NAME, HWC3
...
class UnimatchDetector(ToMixin, BasePreprocessor):
    ...
    def process(self, input_image: InputImage, image2=None, detect_resolution=512, upscale_method="INTER_CUBIC", return_flow=False, pred_bwd_flow=False, pred_bidir_flow=False, **kwargs) -> OutputImage:
        ...
        if image2 is None:
            raise ValueError("UnimatchDetector requires a second image (image2 parameter)")
        
        if not isinstance(image2, np.ndarray):
            image2 = self._load_image(image2)
            image2 = np.asarray(image2, dtype=np.uint8)
            
        image1, remove_pad1 = resize_image_with_pad(image1, detect_resolution, upscale_method)
        image2, remove_pad2 = resize_image_with_pad(image2, detect_resolution, upscale_method)
