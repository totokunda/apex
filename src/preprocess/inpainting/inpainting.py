    ...
    def __call__(
        ...
        if mode == "salient":
            ...
            out_image, out_mask = self.apply_plain_mask(
                image_array, mask_result, mask_color
            )
            ...
            resize_mask = cv2.resize(
                mask_result, (256, 256), interpolation=cv2.INTER_NEAREST
            )
            ...
            out_image, out_mask = self.apply_seg_mask(
                image_array, out_mask, mask_color, mask_cfg
            )
    ...
    def __call__(
        ...
        if mode == "salient":
            ...
            mask_data = self.sam2_model(
                video=video, mask=salient_mask, task_type="mask"
            )
            ...
            if bbox is None:
                raise ValueError("Could not extract bounding box from salient mask")
            mask_data = self.sam2_model(
                video=video, input_box=bbox, task_type="input_box"
            )
            out_frames, out_masks = self.apply_seg_mask(
                mask_data, frames, mask_color, mask_cfg, return_frame
            )
