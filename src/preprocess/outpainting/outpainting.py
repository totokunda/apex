...
class OutpaintingPreprocessor(BasePreprocessor):
    def __init__(self, mask_blur=0, random_cfg=None, return_mask=False, return_source=True, 
                 keep_padding_ratio=8, mask_color=0, **kwargs):
    ...
    def __call__(self,
        ...
        if self.random_cfg:
            ...
            direction = random.sample(
                direction_range,
                random.choice(list(range(1, len(direction_range) + 1))))
        ...
        if mask is None:
            ...
            draw.rectangle(
                (left + (self.mask_blur * 2 if left > 0 else 0), up +
                 (self.mask_blur * 2 if up > 0 else 0), mask_img.width - right -
                 (self.mask_blur * 2 if right > 0 else 0) - 1, mask_img.height - down -
...
class OutpaintingVideoPreprocessor(OutpaintingPreprocessor, BasePreprocessor):
    ...
    def __call__(self, frames: Union[List[Image.Image], List[str], str],
        ...
        for frame in frames:
            anno_frame = super().__call__(frame, expand_ratio=expand_ratio, mask=mask, 
                                        direction=direction, return_mask=return_mask, 
                                        return_source=return_source, mask_color=mask_color)
...
class OutpaintingInnerVideoPreprocessor(OutpaintingInnerPreprocessor, BasePreprocessor):
    ...
    def __call__(self, frames: Union[List[Image.Image], List[str], str],
        ...
        for frame in frames:
            anno_frame = super().__call__(frame, expand_ratio=expand_ratio, direction=direction, 
                                        return_mask=return_mask, return_source=return_source, 
