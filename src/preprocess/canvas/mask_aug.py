...
class MaskAugAnnotator:
    def __init__(self):
        ...
        self.mask_cfg = [
            ...
            {"mode": "bbox", "proba": 0.1},
            {
                "mode": "bbox_expand",
                "proba": 0.1,
                "kwargs": {"min_expand_ratio": 0.2, "max_expand_ratio": 0.5},
            },
        ]

    def __call__(self, mask, mask_cfg=None):
    ...
    def get_expand_params(self, mask_kwargs):
        if "expand_ratio" in mask_kwargs:
            ...
            expand_ratio = random.uniform(
                mask_kwargs["min_expand_ratio"], mask_kwargs["max_expand_ratio"]
            )
        else:
            expand_ratio = 0.3
    ...
    def get_mask_func(self, mask_cfg):
        ...
        if mode == "original":
            ...
            expand_ratio, expand_iters, expand_lrtp = self.get_expand_params(
                mask_kwargs
            )
            mask_func = partial(
                self.generate_mask,
            ...
            clockwise = (
                ...
                if "clockwise" not in mask_kwargs
                else mask_kwargs["clockwise"]
            )
    ...
    def generate_mask(
        ...
        if expand_ratio:
            bin_mask = self.rand_expand_mask(
                bin_mask, bbox, h, w, expand_ratio, expand_iters, expand_lrtp
            )
        return bin_mask

    @staticmethod
    def rand_expand_mask(
