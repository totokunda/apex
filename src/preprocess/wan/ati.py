...
import numpy as np
import torch
from src.preprocess.base import BasePreprocessor, PreprocessorType, preprocessor_registry
...
@preprocessor_registry("wan.ati")
class ATIPreprocessor(BasePreprocessor):
    def __init__(self, **kwargs):
        super().__init__(**kwargs, preprocessor_type=PreprocessorType.OTHER)

    def __call__(self, tracks: np.ndarray | str, width: int, height: int, quant_multi: int = 8, **kwargs):
