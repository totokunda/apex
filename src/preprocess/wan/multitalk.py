...
class MultiTalkPreprocessor(BasePreprocessor, LoaderMixin, OffloadMixin):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if audio_paths is not None:
            ...
            raise ValueError("Must provide either audio paths, embeddings, or audio files")
    ...
    def _process_audio_files(
        ...
        if self.wav2vec_model is None:
            raise ValueError("Wav2Vec2 model not available. Please provide pre-computed embeddings.")
    ...
    def _extract_audio_from_video(self, video_path: str, sample_rate: int) -> np.ndarray:
    ...
    def _extract_audio_features(self, audio_array: np.ndarray, num_frames: int) -> torch.Tensor:
    ...
    def _generate_human_masks(
        ...
        if human_num == 1:
            ...
            if bbox is not None:
                ...
                lefty_min, lefty_max = int((width//2) * face_scale), int((width//2) * (1 - face_scale))
