...
import torch
from src.preprocess.util import HWC3, resize_image_with_pad, common_input_validate, custom_hf_download, BDS_MODEL_NAME
...
class AnimeFaceSegmentor(ToMixin, BasePreprocessor):
    def __init__(self, model, seg_model):
        self.model = model
        self.seg_model = seg_model
        self.device = get_torch_device()
        self.to_device(self.model, self.seg_model.net, device=self.device)
    ...
    def from_pretrained(cls, pretrained_model_or_path=BDS_MODEL_NAME, filename="UNet.pth", seg_filename="isnetis.ckpt"):
        ...
        seg_model.net.eval()
        seg_model.net.to('cpu')  # Explicitly move to CPU
        return cls(model, seg_model)


    def process(self, input_image:InputImage, detect_resolution=512, upscale_method="INTER_CUBIC", remove_background=True, **kwargs) -> OutputImage:
