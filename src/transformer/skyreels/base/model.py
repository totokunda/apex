...
class SkyReelsTimeTextImageFPSEmbedding(nn.Module):
    def __init__(
        self,
        dim: int,
        time_freq_dim: int,
        ...
        if inject_sample_info:
            ...
            self.fps_projection = nn.Sequential(
                nn.Linear(dim, dim), nn.SiLU(), nn.Linear(dim, dim * 6)
            )
        ...
        if image_embed_dim is not None:
            self.image_embedder = SkyReelsImageEmbedding(
    ...
    def forward(
        ...
        grid_sizes: Optional[Tuple[int, int]] = None,
    ):
        ...
        if self.inject_sample_info:
            ...
            fps_embed = self.fps_embedding(encoder_hidden_states_fps)
            if _flag_df:
                fps_proj = (
                    self.fps_projection(fps_embed)
                    .unflatten(1, (6, self.dim))
                    .repeat(f, 1, 1)
                )
            else:
                fps_proj = self.fps_projection(fps_embed).unflatten(1, (6, self.dim))

            timestep_proj = timestep_proj + fps_proj

        if _flag_df and grid_sizes is not None:
            ...
            timestep_proj = timestep_proj.repeat(
                1, 1, grid_sizes[1], grid_sizes[2], 1, 1
            ).flatten(1, 3)
...
class SkyReelsTransformerBlock(nn.Module):
    def __init__(
        ...
        self.attn1 = Attention(
            ...
            out_bias=True,
            processor=SkyReelsAttnProcessor(),
        )
    ...
    def unset_causal_attention(self):
        self.flag_causal_attention = False
        self.attention_mask = None

    def set_causal_attention(self, attention_mask: torch.Tensor):
    ...
    def forward(
        ...
        if temb.dim() == 3:
            ...
            scale_shift_table = self.scale_shift_table.unsqueeze(2)
            shift_msa, scale_msa, gate_msa, c_shift_msa, c_scale_msa, c_gate_msa = (
                scale_shift_table + temb.float()
            ).chunk(6, dim=1)
            # squeeze all dims except the last one
...
@TRANSFORMERS_REGISTRY("skyreels.base")
class SkyReelsTransformer3DModel(
    ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin, CacheMixin
):
    r"""
    A Transformer model for video-like data used in the SkyReels model.
    ...
    _no_split_modules = ["SkyReelsTransformerBlock"]
    ...
    def __init__(
        ...
        self.rope = SkyReelsRotaryPosEmbed(
        ...
        self.condition_embedder = SkyReelsTimeTextImageFPSEmbedding(
            dim=inner_dim,
            ...
            inject_sample_info=inject_sample_info,
        )
    ...
    def set_causal_attention(self, causal_block_size, frame_num, height, width, device):
        ...
        casual_mask = casual_mask.view(frame_num, 1, 1, frame_num, 1, 1).to(device)
        casual_mask = casual_mask.repeat(1, height, width, 1, height, width)
        casual_mask = casual_mask.reshape(
            frame_num * height * width, frame_num * height * width
        )
        self.causal_attention_mask = casual_mask.unsqueeze(0).unsqueeze(0)
        for block in self.blocks:
            block.set_causal_attention(self.causal_attention_mask)

    def unset_causal_attention(self):
        self.flag_causal_attention = False
        self.causal_attention_mask = None
        for block in self.blocks:
            block.unset_causal_attention()
