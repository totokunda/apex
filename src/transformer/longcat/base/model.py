...
class LongCatVideoTransformer3DModel(
    ...
    def __init__(
        ...
        self.lora_dict = {}
        self.active_loras = []
        attention_register._add("bsa", lambda q, k, v, **kwargs: None, allow=True, available=True)
        
    
    def set_adapters(self, adapter_names, weights):
        non_longcat_adapter_names = [adapter_name for adapter_name in adapter_names if adapter_name not in ["cfg_step_lora", "refinement_lora"]]
        print(f"non_longcat_adapter_names: {non_longcat_adapter_names}")
        if non_longcat_adapter_names:
            super().set_adapters(non_longcat_adapter_names, weights)
             
    def load_lora_adapter(self, pretrained_model_name_or_path_or_dict, prefix="transformer", hotswap: bool = False, **kwargs):
        adapter_name = kwargs.get("adapter_name", "lora")
        if adapter_name == "cfg_step_lora":
            self._load_lora_adapter_for_longcat(pretrained_model_name_or_path_or_dict, prefix="transformer", hotswap=hotswap, **kwargs)
        elif adapter_name == "refinement_lora":
            self._load_lora_adapter_for_longcat(pretrained_model_name_or_path_or_dict, prefix="transformer", hotswap=hotswap, **kwargs)
        else:
            super().load_lora_adapter(pretrained_model_name_or_path_or_dict, prefix="transformer", hotswap=hotswap, **kwargs)
    
    # This is a custom method for LongCat to load LoRAs.
    def _load_lora_adapter_for_longcat(self, pretrained_model_name_or_path_or_dict, prefix="transformer", hotswap: bool = False, **kwargs):
        multiplier = kwargs.get("multiplier", 1.0)
        lora_network_dim = kwargs.get("lora_network_dim", 128)
        lora_network_alpha = kwargs.get("lora_network_alpha", 64)
        adapter_name = kwargs.get("adapter_name", "lora")
        if isinstance(pretrained_model_name_or_path_or_dict, str):
            lora_network_state_dict_loaded = load_file(pretrained_model_name_or_path_or_dict, device="cpu")
        else:
            lora_network_state_dict_loaded = pretrained_model_name_or_path_or_dict
        ...
        self.lora_dict[adapter_name] = lora_network
        self.enable_loras(list(self.lora_dict.keys()))
