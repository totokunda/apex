...
from .attention import WanAttnProcessor, WanAnimateFaceBlockAttnProcessor
...
class WanAnimateFaceEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x.unflatten(1, (self.num_heads, -1)).flatten(
            0, 1
        )  # [B, N * C, T] --> [B * N, C, T]
...
class WanAnimateTransformer3DModel(
    ...
    def __init__(
        ...
        pos_embed_seq_len: Optional[int] = None,
        motion_encoder_channel_sizes: Optional[
            Dict[str, int]
        ] = None,  # Start of Wan Animate-specific args
        ...
        if in_channels is None and latent_channels is not None:
            ...
            assert (
                in_channels == 2 * latent_channels + 4
            ), "in_channels should be 2 * latent_channels + 4"
        else:
            raise ValueError(
                "At least one of `in_channels` and `latent_channels` must be supplied."
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if (
            pose_hidden_states is not None
            and pose_hidden_states.shape[2] + 1 != hidden_states.shape[2]
        ):
            raise ValueError(
        ...
        for block_idx, block in enumerate(self.blocks):
            ...
            if block_idx % self.config.inject_face_latents_blocks == 0:
                face_adapter_block_idx = (
                    block_idx // self.config.inject_face_latents_blocks
                )
        ...
        hidden_states = (hidden_states * (1 + scale) + shift).to(
            dtype=hidden_states_original_dtype
        )
