...
class WanLayerNorm(nn.LayerNorm):
    ...
    def forward(self, x):
        ...
        Args:
            x(Tensor): Shape [B, L, C]
        """
        # Run LayerNorm in fp32 for numerical stability and to avoid bf16/fp16 kernel
        # dtype mismatches under outer autocast contexts, then cast back.
        orig_dtype = x.dtype
...
class WanSelfAttention(nn.Module):
    ...
    def forward(self, x, seq_lens, grid_sizes, freqs, dtype=torch.bfloat16, t=0):
        ...
        x = attention_register.call(
            q.to(dtype).transpose(1, 2), 
            k.to(dtype).transpose(1, 2), 
...
class WanI2VCrossAttention(WanSelfAttention):
    ...
    def forward(self, x, context, context_lens, dtype=torch.bfloat16, t=0):
        ...
        img_x = attention_register.call(
            q.to(dtype).transpose(1, 2), 
            k_img.to(dtype).transpose(1, 2), 
            v_img.to(dtype).transpose(1, 2), 
            k_lens=None
        )
...
class WanCrossAttention(WanSelfAttention):
    def forward(self, x, context, context_lens, dtype=torch.bfloat16, t=0):
        ...
        v = self.v(context.to(dtype)).view(b, -1, n, d)
        # compute attention
        x = attention_register.call(q.to(dtype).transpose(1, 2), k.to(dtype).transpose(1, 2), v.to(dtype).transpose(1, 2), k_lens=context_lens)
        x = x.transpose(1, 2)
        # output
        x = x.flatten(2)
        x = self.o(x.to(dtype))
...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        def cross_attn_ffn(x, context, context_lens, e):
            # cross-attention
            x = x + self.cross_attn(self.norm3(x.float()).type_as(x), context, context_lens, dtype, t=t)

            # ffn function
            temp_x = self.norm2(x.float()).type_as(x) * (1 + e[4]) + e[3]
            temp_x = temp_x.to(dtype)
...
class WanTransformer3DModel(ModelMixin, ConfigMixin, FromOriginalModelMixin, PeftAdapterMixin):
    r"""
