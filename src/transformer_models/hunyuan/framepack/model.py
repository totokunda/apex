...
from src.transformer_models.hunyuan.base.model import (
...
class HunyuanVideoHistoryPatchEmbed(nn.Module):
    def __init__(self, in_channels: int, inner_dim: int):
        super().__init__()
        self.proj = nn.Conv3d(
            in_channels, inner_dim, kernel_size=(1, 2, 2), stride=(1, 2, 2)
        )
        self.proj_2x = nn.Conv3d(
            in_channels, inner_dim, kernel_size=(2, 4, 4), stride=(2, 4, 4)
        )
        self.proj_4x = nn.Conv3d(
            in_channels, inner_dim, kernel_size=(4, 8, 8), stride=(4, 8, 8)
        )
...
class HunyuanVideoFramepackTransformer3DModel(
    ...
    def __init__(
        ...
        if has_clean_x_embedder:
            self.clean_x_embedder = HunyuanVideoHistoryPatchEmbed(
        ...
        self.image_projection = (
            FramepackClipVisionProjection(image_proj_dim, inner_dim)
            if has_image_proj
            else None
        )
    ...
    def forward(
        ...
        if indices_latents is None:
            indices_latents = (
                torch.arange(0, num_frames).unsqueeze(0).expand(batch_size, -1)
            )
        ...
        encoder_attention_mask = torch.cat(
            [attention_mask_image, encoder_attention_mask], dim=1
        )
        ...
        if batch_size == 1:
            encoder_hidden_states = encoder_hidden_states[
                :, : effective_condition_sequence_length[0]
            ]
            attention_mask = None
        else:
    ...
    def _pack_history_states(
        ...
        if latents_clean is not None and image_rotary_emb_clean is not None:
            ...
            image_rotary_emb[0] = torch.cat(
                [image_rotary_emb_clean[0], image_rotary_emb[0]], dim=0
            )
            image_rotary_emb[1] = torch.cat(
                [image_rotary_emb_clean[1], image_rotary_emb[1]], dim=0
            )
        ...
        if latents_history_2x is not None and image_rotary_emb_history_2x is not None:
            ...
            image_rotary_emb[0] = torch.cat(
                [image_rotary_emb_history_2x[0], image_rotary_emb[0]], dim=0
            )
            image_rotary_emb[1] = torch.cat(
                [image_rotary_emb_history_2x[1], image_rotary_emb[1]], dim=0
            )
        ...
        if latents_history_4x is not None and image_rotary_emb_history_4x is not None:
            ...
            image_rotary_emb[0] = torch.cat(
                [image_rotary_emb_history_4x[0], image_rotary_emb[0]], dim=0
            )
            image_rotary_emb[1] = torch.cat(
                [image_rotary_emb_history_4x[1], image_rotary_emb[1]], dim=0
            )
    ...
    def _pad_rotary_emb(
        ...
        freqs_cos = (
            freqs_cos.unsqueeze(0).permute(0, 2, 1).unflatten(2, (-1, height, width))
        )
        freqs_sin = (
            freqs_sin.unsqueeze(0).permute(0, 2, 1).unflatten(2, (-1, height, width))
        )
