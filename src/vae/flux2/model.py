...
class AutoencoderKLFlux2(ModelMixin, AutoencoderMixin, ConfigMixin, FromOriginalModelMixin, PeftAdapterMixin):
    ...
    def normalize_latents(self, latents: torch.Tensor) -> torch.Tensor:
        latents_bn_mean = self.bn.running_mean.view(1, -1, 1, 1).to(latents.device, latents.dtype)
        latents_bn_std = torch.sqrt(self.bn.running_var.view(1, -1, 1, 1) + self.config.batch_norm_eps)
        latents = (latents - latents_bn_mean) / latents_bn_std
        return latents
    
    def denormalize_latents(self, latents: torch.Tensor) -> torch.Tensor:
        latents_bn_mean = self.bn.running_mean.view(1, -1, 1, 1).to(latents.device, latents.dtype)
        latents_bn_std = torch.sqrt(self.bn.running_var.view(1, -1, 1, 1) + self.config.batch_norm_eps).to(
