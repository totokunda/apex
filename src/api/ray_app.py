...
if not ray.is_initialized():
    ...
    try:
        ...
        logger.info(f"Ray dashboard available at http://localhost:{settings.ray_dashboard_port}")
    except Exception as e:
        if "Cannot include dashboard" in str(e):
            logger.warning("Ray dashboard packages not installed, starting without dashboard")
            logger.info("To enable dashboard: pip install 'ray[default,dashboard]' aiohttp aiohttp-cors grpcio")
            ray.init(
                ignore_reinit_error=True,
                num_cpus=os.cpu_count(),
