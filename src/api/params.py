...
def validate_and_convert_params(
    ...
    for param_name, param_value in params.items():
        ...
        if param_name not in param_defs:
            logger.warning(f"Parameter '{param_name}' not found in definitions, passing through as-is")
    ...
    if isinstance(value, str):
        ...
        for valid_value in valid_values:
            if isinstance(valid_value, (int, float)):
                try:
                    converted = float(value) if isinstance(valid_value, float) else int(value)
