...
class HuMoEngine(WanShared):
    ...
    def load_image_latents(self, image: InputImage | List[InputImage], size, device=None):
        # Load size.
        h, w = size[1], size[0]
    ...
    def audio_emb_enc(self, audio_emb, wav_enc_type="whisper"):
        if wav_enc_type == "wav2vec":
            ...
            feat0 = self.linear_interpolation_fps(audio_emb[:, :, 0: 8].mean(dim=2), 50, 25)
            feat1 = self.linear_interpolation_fps(audio_emb[:, :, 8: 16].mean(dim=2), 50, 25)
            feat2 = self.linear_interpolation_fps(audio_emb[:, :, 16: 24].mean(dim=2), 50, 25)
            feat3 = self.linear_interpolation_fps(audio_emb[:, :, 24: 32].mean(dim=2), 50, 25)
    ...
    def forward_tia(self, latents, timestep, t, step_change, arg_tia, arg_ti, arg_i, arg_null, scale_a, scale_t):
        pos_tia, _ = self.parse_output(self.transformer(
            latents, t=timestep, **arg_tia
            ))
        torch.cuda.empty_cache()

        pos_ti, _ = self.parse_output(self.transformer(
            latents, t=timestep, **arg_ti
            ))
        torch.cuda.empty_cache()

        if t > step_change:
            neg, _ = self.parse_output(self.transformer(
    ...
    def forward_ta(self, latents, timestep, arg_ta, arg_t, arg_null, scale_a, scale_t):
        pos_ta, _ = self.parse_output(self.transformer(
            latents, t=timestep, **arg_ta
            ))
        torch.cuda.empty_cache()

        pos_t, _ = self.parse_output(self.transformer(
    ...
    def forward_tia_small(self, latents, latents_ref, latents_ref_neg, timestep, arg_t, arg_ta, arg_null, scale_a, scale_t):
        ...
        noise_pred = scale_a * (pos_tia - pos_ta) + \
    ...
    def forward_ta_small(self, latents, latents_ref_neg, timestep, arg_t, arg_ta, arg_null, scale_a, scale_t):
        neg = self.transformer(
            [torch.cat([latent[:,:-latent_ref_neg.shape[1]], latent_ref_neg], dim=1) for latent, latent_ref_neg in zip(latents, latents_ref_neg)], t=timestep, **arg_null
            )[0][0]
    ...
    def run(self,
        ...
        if image is not None:
            ...
            generation_mode = "TA"
            latents_ref = [torch.zeros(16, 1, height//8, width//8).to(self.device, dtype=transformer_dtype)]
        ...
        zero_video_tensor = torch.zeros(1, 3, frame_num, noise.shape[3]*8, noise.shape[4]*8).to(self.device)
        zero_vae = self.vae_encode(zero_video_tensor, offload=offload, sample_mode="mode")[0].squeeze(0)
        transformer_config = self.load_config_by_type("transformer")
        noise_shape = noise.shape[2:]
        noise_shape = [noise_shape[i] // transformer_config['patch_size'][i] for i in range(len(noise_shape))]
        ...
        with amp.autocast(device_type=self.device.type, dtype=transformer_dtype):
            if not small_model:
                ...
                for i, t in enumerate(tqdm(timesteps, desc="Sampling", total=len(timesteps))):
                    ...
                    if generation_mode == "TIA":
                        noise_pred = self.forward_tia(latents, timestep, t, step_change, 
                                                          arg_tia, arg_ti, arg_i, arg_null, guidance_scale_a, guidance_scale_t)
                    elif generation_mode == "TA":
                        noise_pred = self.forward_ta(latents, timestep, arg_ta, arg_t, arg_null, guidance_scale_a, guidance_scale_t)
                ...
                for _, t in enumerate(tqdm(timesteps, desc="Sampling", total=num_inference_steps)):
                    ...
                    timestep = torch.stack(timestep)
                    if generation_mode == "TIA":
                        noise_pred = self.forward_tia_small(latents, latents_ref, latents_ref_neg, timestep, arg_t, arg_ta, arg_null, guidance_scale_a, guidance_scale_t)
                    elif generation_mode == "TA":
                        noise_pred = self.forward_ta_small(latents, latents_ref_neg, timestep, arg_t, arg_ta, arg_null, guidance_scale_a, guidance_scale_t)
