...
class WanFunVACEEngine(WanShared):
    ...
    def vace_encode_frames(self, frames, ref_images, masks=None, vae=None):
        ...
        if masks is None:
            ...
            inactive = torch.stack([i * (1 - m) + 0 * m for i, m in zip(frames, masks)], dim=0)
            reactive = torch.stack([i * m + 0 * (1 - m) for i, m in zip(frames, masks)], dim=0)
    ...
    def get_image_latent(self, ref_image=None, sample_size=None, padding=False):
        if ref_image is not None:
            ...
            ref_image = self._aspect_ratio_resize(ref_image, max_area=math.prod(sample_size))[0]
    ...
    def get_image_to_video_latent(self, validation_image_start, validation_image_end, video_length, sample_size):
        if validation_image_start is not None and validation_image_end is not None:
            ...
            image_start = self._aspect_ratio_resize(image_start, max_area=math.prod(sample_size))[0]
            clip_image = self._aspect_ratio_resize(clip_image, max_area=math.prod(sample_size))[0]
           
            image_end = self._load_image(validation_image_end)
            image_end = self._aspect_ratio_resize(image_end, max_area=math.prod(sample_size))[0]
    ...
    def get_video_to_video_latent(self, input_video_path, video_length, sample_size, fps=16, validation_video_mask=None, ref_image=None):
        if input_video_path is not None:
            ...
            input_video = [self._aspect_ratio_resize(frame, max_area=math.prod(sample_size))[0] for frame in input_video]
