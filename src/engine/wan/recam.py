...
class WanRecamEngine(WanShared):
    ...
    def create_frame_process(self, height, width):
        self.height = height
        self.width = width
        self.logger.info(f"Creating frame process for height: {height} and width: {width}")
        return v2.Compose([
    ...
    def load_frames(self, video, max_num_frames, start_frame_id, interval, num_frames, frame_process):
        ...
        if total_frames < max_num_frames or total_frames - 1 < start_frame_id + (num_frames - 1) * interval:
    ...
    def run(
        ...
        prompt: List[str] | str,
        camera_extrinsics: str | np.ndarray | torch.Tensor = "assets/pose/camera_extrinsics.json",
        ...
        **kwargs,
        ...
        preprocessed_video = self.load_frames(source_video, self.max_num_frames, start_frame_id, self.frame_interval, self.num_frames, frame_process)
        ...
        if isinstance(camera_extrinsics, str):
            ...
            camera_extrinsics = torch.from_numpy(camera_extrinsics).to(
                self.device
            )
        else:
        ...
        with self._progress_bar(
            ...
            for i, t in enumerate(timesteps):
                timestep = t.expand(latents.shape[0]).to(dtype=transformer_dtype, device=self.device)
                latent_model_input = torch.cat(
                    [latents, source_latents], dim=2
