...
class StepVideoBaseEngine:
    ...
    def _load_optimus(self):
        if hasattr(torch.ops, "Optimus") and hasattr(torch.ops.Optimus, "fwd"):
            ...
            if pver.parse(torch_version) >= pver.parse("2.2.0") and torch.cuda.is_available():
                # Check CUDA version
                    ...
                    if library_url:
                        ...
                        try:
                            ...
                            if os.path.exists(final_lib_path):
                                self.logger.info(f"Optimus library already exists at {final_lib_path}")
                            else:
                                # Download the library
                                self.logger.info(f"Downloading Optimus library {library_key} from {library_url}")
                                downloaded_path = self.main_engine._download(library_url, optimus_dir)
                                
                                if downloaded_path and os.path.exists(downloaded_path):
                                    # If the downloaded path is different from our target, copy it
                                    if downloaded_path != final_lib_path:
                                        import shutil
                                        self.logger.info(f"Moving library from {downloaded_path} to {final_lib_path}")
                                        ...
                                        try:
                                            ...
                                            while parent_dir != optimus_dir and os.path.exists(parent_dir):
                                                if not os.listdir(parent_dir):  # Only remove if empty
                                                    os.rmdir(parent_dir)
                                    ...
                                    self.logger.error(f"Failed to download Optimus library")
                                    return False
                            ...
                            if os.path.exists(final_lib_path):
                                ...
                                # Load the library
                                self.logger.info(f"Loading Optimus library from {final_lib_path}")
                                torch.ops.load_library(final_lib_path)
                                
                                # Verify it's loaded correctly
                                if hasattr(torch.ops, "Optimus") and hasattr(torch.ops.Optimus, "fwd"):
                                    self.logger.info("Optimus library loaded successfully")
                                    return True
                                else:
                                    self.logger.warning("Optimus library downloaded but not loaded properly")
                                    return False
                            else:
                                self.logger.error(f"Library file not found at {final_lib_path}")
                                return False
                            ...
                            if "failed to map segment" in str(e):
                                self.logger.error("This error often indicates permission issues or incompatible library. Try:")
                                self.logger.error("1. Check if /tmp or target directory has execute permissions")
                                self.logger.error("2. Ensure sufficient disk space")
                                self.logger.error("3. Verify CUDA and torch versions match the library")
                            return False
                    else:
                        self.logger.warning(f"No Optimus library available for torch version {torch_version}")
                        return False
                else:
                    cuda_ver_str = cuda_version if cuda_version else "unknown"
                    self.logger.info(f"CUDA version {cuda_ver_str} is too old, need CUDA 12+ for Optimus")
                    return False
            else:
                self.logger.info(f"Torch version {torch_version} is too old or CUDA not available, need torch 2.2+ with CUDA for Optimus")
                return False
    ...
    def vae_decode(
        ...
        video = self.vae.decode(denormalized_latents, device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride, smooth_scale=smooth_scale, return_dict=False)[0]
        if offload:
