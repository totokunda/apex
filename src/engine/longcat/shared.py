...
class LongCatShared(BaseEngine):
    ...
    def prepare_latents(
        ...
        if image is not None or video is not None:
            ...
            for i in range(batch_size):
                ...
                latent = self.vae_encode(encoded_input, sample_mode="sample", sample_generator=gen, offload=offload)
    ...
    def _cache_clean_latents(self, cond_latents, model_max_length, offload_kv_cache, device, dtype):
        ...
        empty_embeds = torch.zeros([cond_latents.shape[0], 1, model_max_length, text_encoder_config.d_model], device=device, dtype=dtype)
