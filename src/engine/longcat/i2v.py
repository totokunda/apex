...
class LongCatI2VEngine(LongCatShared):
    """LongCat Image-to-Video Engine Implementation"""
    ...
    def run(
        ...
        **kwargs,
        ...
        """Image-to-video generation following LongCatPipeline.
        ...
        # Keep original image size for post-refine resizing, matching the demo
        ref_image = image[0] if isinstance(image, list) and image else image
        target_size = ref_image.size if isinstance(ref_image, Image.Image) else None

        height, width = self.get_condition_shape(
            image, resolution, scale_factor_spatial=scale_factor_spatial
        )
        ...
        if enable_refine:
            ...
            if refine_engine is None:
                self.logger.warning(
                    "[LongCatI2VEngine] `refine=True` was requested but no `refine` sub-engine "
