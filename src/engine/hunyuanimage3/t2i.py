...
from src.transformer.hunyuanimage3.base.model import HunyuanStaticCache, build_batch_2d_rope
...
class HunyuanImage3T2IEngine(BaseEngine):
    ...
    def _get_latents(self, batch_size, latent_channel, image_size, dtype, device, generator, latents=None):
    ...
    def prepare_model_inputs(
        ...
        #   -- 2.2 Prompt, cot text, system prompt
            ...
            if mode == "gen_image":
                batch_gen_image_info = [self.hy_image_processor.build_image_info(image_size) for _ in range(batch_size)]
        ...
        if mode == "gen_image":
            ...
            max_cache_len = output.tokens.shape[1] + default(max_new_tokens, max_sequence_length)
