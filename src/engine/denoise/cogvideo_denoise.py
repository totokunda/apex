...
class CogVideoDenoise:
    def __init__(self, denoise_type: CogVideoDenoiseType = CogVideoDenoiseType.T2V, *args, **kwargs):
    ...
    def denoise(self, *args, **kwargs) -> torch.Tensor:
        ...
        additional_frames = kwargs.get("additional_frames", 0)
        ...
        with self._progress_bar(total=num_inference_steps, desc=f"Denoising {mode_desc}") as pbar:
            ...
            for i, t in enumerate(timesteps):
                ...
                if self.denoise_type == CogVideoDenoiseType.I2V and image_latents is not None:
                    # Concatenate with image latents for I2V
                    latent_image_input = torch.cat([image_latents] * 2) if do_classifier_free_guidance else image_latents
                    latent_model_input = torch.cat([latent_model_input, latent_image_input], dim=2)
                elif self.denoise_type == CogVideoDenoiseType.CONTROL and control_video_latents is not None:
                ...
                # Predict noise
                with self.transformer.cache_context("cond_uncond" if do_classifier_free_guidance else None):
                ...
                if (hasattr(scheduler, 'step') and 
                    'old_pred_original_sample' in scheduler.step.__code__.co_varnames and
                    self.denoise_type != CogVideoDenoiseType.CONTROL):  # Control mode uses simpler scheduler step
                    # For DPM-solver++ (T2V, I2V, V2V)
                    ...
                    # For other schedulers or Control mode
                    latents = scheduler.step(noise_pred, t, latents, return_dict=False)[0]

                latents = latents.to(noise_pred_kwargs.get("encoder_hidden_states").dtype)
