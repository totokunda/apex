    ...
    def df_run(
        ...
        if image is not None:
            ...
            prefix_latent = self.vae_encode(
                preprocessed_image, offload=offload, dtype=torch.float32
            )
        ...
        if end_image is not None:
            ...
            end_latent = self.vae_encode(
                preprocessed_end_image, offload=offload, dtype=torch.float32
            )
            ...
            with self._progress_bar(total=n_iter, desc="Generating video") as pbar:
                for i in range(n_iter):
                    if output_video is not None:
                        ...
                        prefix_latent = self.vae_encode(
                            prefix_video, offload=offload, dtype=torch.float32
                        )
