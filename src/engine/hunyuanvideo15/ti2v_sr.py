...
class HunyuanVideo15TI2VSRengine(HunyuanVideo15TI2VEngine):
    ...
    def _load_sr_transformer(self):
        ...
        if getattr(self, "sr_transformer", None) is None:
            raise ValueError("SR transformer component not found; add a transformer component named 'sr_transformer'.")
    ...
    def _load_upsampler(self, target_resolution: str):
        ...
        raise ValueError("Upsampler component not found; add an 'upsampler' component for SR.")

    def _bucket_map(self, lr_bucket: Tuple[int, int], base_resolution: str, target_resolution: str) -> Tuple[int, int]:
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        """
        sr_resolution = sr_resolution.lower() if isinstance(sr_resolution, str) else None
        ...
        if sr_resolution is None:
            latents_to_decode = base_latents if base_latents.ndim == 5 else base_latents.unsqueeze(2)
        ...
        guidance_scale = sr_guidance_scale if sr_guidance_scale is not None else sr_cfg.get("guidance_scale", 1.0)
        embedded_guidance_scale = sr_cfg.get("embedded_guidance_scale", None)
        num_steps = sr_num_inference_steps or sr_cfg.get("num_inference_steps", 6 if sr_resolution == "720p" else 8)
        ...
        sr_use_meanflow = bool(getattr(getattr(self.sr_transformer, "config", None), "use_meanflow", False))
        ...
        lq_latents = self.upsampler(lq_latents.to(dtype=torch.float32, device=self.device))
        ...
        vision_states = self._prepare_vision_states(
            None if reference_image is None else np.array(reference_image), sr_resolution, latents, device
        )
        ...
        with self._progress_bar(total=num_inference_steps, desc=f"SR {sr_resolution}") as pbar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = (
                    torch.cat([latents_concat] * 2) if guidance_scale > 1.0 else latents_concat
                )
